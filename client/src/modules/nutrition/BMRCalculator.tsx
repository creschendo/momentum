...
function normalizeTdeeResult(value: Record<string, unknown>): TdeeResult {
  ...
  return {
    bmr: toNumber(value.bmr),
    tdee: toNumber(value.tdee),
    calorieTargets: {
      maintain: toNumber(targets.maintain),
      mildWeightLoss: toNumber(targets.mildWeightLoss),
      weightLoss: toNumber(targets.weightLoss),
      extremeWeightLoss: toNumber(targets.extremeWeightLoss)
    },
    macros: {
      protein_g: toNumber(macros.protein_g),
      carbs_g: toNumber(macros.carbs_g),
      fat_g: toNumber(macros.fat_g)
    }
  };
}
...
export default function BMRCalculator() {
  ...
  const [system, setSystem] = useState<UnitSystem>('imperial'); // 'metric' or 'imperial'
  const [result, setResult] = useState<TdeeResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedGoal, setSelectedGoal] = useState<GoalKey | null>(null);
  const [confirmingGoal, setConfirmingGoal] = useState<GoalOption | null>(null);

  // Load selected goal from context on mount
  useEffect(() => {
    if (calorieGoal) {
      setSelectedGoal(calorieGoal.goalKey as GoalKey);
    }
  }, [calorieGoal]);
  ...
  const handleCalculate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setResult(normalizeTdeeResult(data));
    } catch (err: unknown) {
  ...
  const activityLevels: Array<{ value: number; label: string }> = [
  ...
  const goals: GoalOption[] = [
    { key: 'maintain', label: 'Maintain Weight', value: result?.calorieTargets?.maintain || result?.tdee || 0, color: '#48bb78', desc: 'no change' },
    { key: 'mildWeightLoss', label: 'Mild Loss', value: result?.calorieTargets?.mildWeightLoss || 0, color: '#4299e1', desc: '0.5 lb/week' },
    { key: 'weightLoss', label: 'Weight Loss', value: result?.calorieTargets?.weightLoss || 0, color: '#3182ce', desc: '1 lb/week' },
    { key: 'extremeWeightLoss', label: 'Extreme Weight Loss', value: result?.calorieTargets?.extremeWeightLoss || 0, color: '#ed8936', desc: '1.5 lb/week' }
  ];
