...
export default function WaterTracker() {
  ...
  const { summary, loading, addEntry, resetEntries } = useWater();
  const [value, setValue] = useState<number>(250);
  const [goalMl, setGoalMl] = useState<number>(2000);
  const [animatingDroplets, setAnimatingDroplets] = useState<Droplet[]>([]);
          ...
          <div>
            ...
            {loading ? (
              ...
              <div style={{ fontSize: 14, color: isDark ? theme.text : '#2d3748', fontWeight: 500 }}>
                {summary?.start ? new Date(summary.start).toLocaleDateString() : 'No data'}
              </div>
            )}
          </div>
