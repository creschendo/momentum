...
export default function Splits() {
  ...
  const [expandedSplit, setExpandedSplit] = useState<EntityId | null>(null);
  const [expandedDay, setExpandedDay] = useState<EntityId | null>(null);
  
  // Lift forms state
  const [liftForms, setLiftForms] = useState<Record<string, LiftForm>>({});
  const [liftWeightErrors, setLiftWeightErrors] = useState<Record<string, boolean>>({});
  
  // Cardio forms state
  const [cardioForms, setCardioForms] = useState<Record<string, CardioForm>>({});

  // Single operation state - ensures only one operation at a time
  const [activeOperation, setActiveOperation] = useState<string | null>(null);
  
  // Drag state for reordering days
  const [draggedDay, setDraggedDay] = useState<DraggedDayState | null>(null);
  const [dragOverSplitId, setDragOverSplitId] = useState<EntityId | null>(null);
  const [reorderedDays, setReorderedDays] = useState<Record<string, DayItem[]>>({});
  
  // Edit mode state
  const [editingLift, setEditingLift] = useState<LiftItem | null>(null);
  const [editingCardio, setEditingCardio] = useState<CardioItem | null>(null);
  const [editingSplitId, setEditingSplitId] = useState<EntityId | null>(null);
  const [editingSplitTitle, setEditingSplitTitle] = useState('');
  const [deleteConfirmSplitId, setDeleteConfirmSplitId] = useState<EntityId | null>(null);
  const [deleteConfirmDayKey, setDeleteConfirmDayKey] = useState<string | null>(null);
  const [editingDayKey, setEditingDayKey] = useState<EntityId | null>(null);
  ...
  useEffect(() => {
    ...
    const handleDocumentMouseDown = (event: MouseEvent) => {
      ...
      if (deleteConfirmRef.current && targetNode && !deleteConfirmRef.current.contains(targetNode)) {
  ...
  const handleAddLift = async (splitId: EntityId, dayId: EntityId, e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await addLift(splitId, dayId, {
        ...
        weight: form.weight || '',
        sets: toInteger(form.sets, 3),
        reps: toInteger(form.reps, 8)
      });
  ...
  const handleAddCardio = async (splitId: EntityId, dayId: EntityId, e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await addCardio(splitId, dayId, {
        ...
        durationMinutes: toInteger(form?.durationMinutes, 20),
  ...
  const setLiftFormValue = (splitId: EntityId, dayId: EntityId, field: keyof LiftForm, value: string | number) => {
    ...
    setLiftForms({
      ...liftForms,
      [formKey]: {
        ...(liftForms[formKey] || { name: '', weight: '', sets: 3, reps: 8 }),
        [field]: value
      }
    });
  };
  ...
  const setCardioFormValue = (splitId: EntityId, dayId: EntityId, field: keyof CardioForm, value: string | number) => {
    ...
    setCardioForms({
      ...cardioForms,
      [formKey]: {
        ...(cardioForms[formKey] || { exerciseName: 'Treadmill', durationMinutes: 20, intensity: 'moderate' }),
        [field]: value
      }
    });
  };
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>, day: DayItem, splitId: EntityId) => {
  ...
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>, splitId: EntityId) => {
    e.preventDefault();
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, targetDay: DayItem, splitId: EntityId) => {
    e.preventDefault();
    ...
    const currentDays = reorderedDays[toKey(splitId)] || currentSplit?.days || [];
    ...
    setReorderedDays({ ...reorderedDays, [toKey(splitId)]: days });

    setDraggedDay(null);
    setDragOverSplitId(null);
  };

  const handleSplitHeaderClick = (event: React.MouseEvent<HTMLElement>, splitId: EntityId) => {
  ...
  const handleDayHeaderClick = (event: React.MouseEvent<HTMLElement>, dayId: EntityId) => {
    if ((event.target as HTMLElement).closest('button, input, textarea, select, label')) return;
              ...
              <div className="tab-swap-fade" style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {split.days && split.days.length > 0 ? (
                    (reorderedDays[toKey(split.id)] || split.days).map((day) => {
                    const lifts = day.lifts ?? [];
                    const cardioEntries = day.cardio ?? [];
                    return (
                    <div
                      key={day.id}
                      draggable
                        ...
                        <div className="tab-swap-fade" style={{ marginTop: 12 }}>
                          <div style={{ padding: 12, backgroundColor: theme.bgSecondary, borderRadius: 6, marginBottom: 12, fontSize: 13, color: theme.text }}>
                            <div style={{ marginBottom: 8 }}>
                              <strong>Lifts:</strong> {lifts.length > 0 ? lifts.map((l) => l.exerciseName).join(', ') : 'None'}
                            </div>
                            <div>
                              <strong>Cardio:</strong> {cardioEntries.length > 0 ? cardioEntries.map((c) => c.exerciseName).join(', ') : 'None'}
                            </div>
                          </div>
                          ...
                          <div style={{ marginBottom: 16 }}>
                            ...
                            {lifts.length > 0 && (
                              <div style={{ marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {lifts.map((lift) => (
                                  <div key={lift.id}>
                                    {editingLift && editingLift.id === lift.id ? (
                                      // Edit Mode
                                      <div style={{ 
                                        padding: 10, 
                                        backgroundColor: theme.bgSecondary, 
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                          ...
                                          <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                                            <button
                                              onClick={() => {
                                                if (lift.id == null) return;
                                                handleUpdateLift(split.id, day.id, lift.id);
                                              }}
                                              style={{
                                                flex: 1,
                                                padding: '6px 12px',
                                      ...
                                      <div style={{ 
                                        ...
                                        <div style={{ display: 'flex', gap: 6 }}>
                                          ...
                                          <button
                                            onClick={() => {
                                              if (lift.id == null) return;
                                              handleDeleteLift(split.id, day.id, lift.id);
                                            }}
                                            style={{
                                              padding: '4px 8px',
                                              backgroundColor: theme.error,
                          ...
                          <div>
                            ...
                            {cardioEntries.length > 0 && (
                              <div style={{ marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {cardioEntries.map((cardio) => (
                                  <div key={cardio.id}>
                                    {editingCardio && editingCardio.id === cardio.id ? (
                                      // Edit Mode
                                      <div style={{ 
                                        padding: 10, 
                                        backgroundColor: theme.bgSecondary, 
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                          ...
                                          <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                                            <button
                                              onClick={() => {
                                                if (cardio.id == null) return;
                                                handleUpdateCardio(split.id, day.id, cardio.id);
                                              }}
                                              style={{
                                                flex: 1,
                                                padding: '6px 12px',
                                      ...
                                      <div style={{ 
                                        ...
                                        <div style={{ display: 'flex', gap: 6 }}>
                                          ...
                                          <button
                                            onClick={() => {
                                              if (cardio.id == null) return;
                                              handleDeleteCardio(split.id, day.id, cardio.id);
                                            }}
                                            style={{
                                              padding: '4px 8px',
                                              backgroundColor: theme.error,
